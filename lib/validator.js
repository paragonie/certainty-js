...
const {SodiumPlus, Ed25519PublicKey} = require('sodium-plus');
...
module.exports = class Validator {
    ...
    async checkEd25519Signature(bundle, backupKey = false) {
        ...
        let publicKey;
        if (backupKey) {
            publicKey = new Ed25519PublicKey(Buffer.from(BACKUP_SIGNING_PUBKEY, 'hex'));
        } else {
            publicKey = new Ed25519PublicKey(Buffer.from(PRIMARY_SIGNING_PUBKEY, 'hex'));
        }
        return await this.sodium.crypto_sign_verify_detached(
    ...
    async validateChronicleContents(bundle, row) {
        ...
        let publicKey = new Ed25519PublicKey(Buffer.from(base64url.parse(row.publickey)));
        if (
        ...
        let signature = Buffer.from(base64url.parse(row.signature));
        if (!await this.sodium.crypto_sign_verify_detached(
            row.contents,
            publicKey,
            signature
        )) {
